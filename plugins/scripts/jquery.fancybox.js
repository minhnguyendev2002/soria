...
(function (window, document, $, undefined) {
    ...
    $.extend(FancyBox.prototype, {
        ...
        init() {
            ...
            let scrollbarWidth = $.fancybox.scrollbarWidth;
            let $scrollDiv;
            let $container;
            let buttonStr;
            ...
            $.each(firstItemOpts.buttons, (index, value) => {
        ...
        translate(obj, str) {
            ...
            return str.replace(/\{\{(\w+)\}\}/g, (match, n) => {
                const value = arr[n];
        ...
        addEvents() {
            ...
            self.$refs.container.on('click.fb-close', '[data-fancybox-close]', (e) => {
                e.stopPropagation();
                e.preventDefault();

                self.close(e);
            }).on('click.fb-prev touchend.fb-prev', '[data-fancybox-prev]', (e) => {
                e.stopPropagation();
                e.preventDefault();

                self.previous();
            }).on('click.fb-next touchend.fb-next', '[data-fancybox-next]', (e) => {
                e.stopPropagation();
                e.preventDefault();

                self.next();
            }).on('click.fb', '[data-fancybox-zoom]', (e) => {
                // Click handler for zoom button
                self[self.isScaledDown() ? 'scaleToActual' : 'scaleToFit']();
            });

            // Handle page scrolling and browser resizing
            $W.on('orientationchange.fb resize.fb', (e) => {
            ...
            if (self.group[self.currIndex].opts.idleTime) {
                ...
                $D.on('mousemove.fb-idle mouseleave.fb-idle mousedown.fb-idle touchstart.fb-idle touchmove.fb-idle scroll.fb-idle keydown.fb-idle', (e) => {
                    self.idleSecondsCounter = 0;
        ...
        scaleToActual(x, y, duration) {
            ...
            let imgPos; let posX; let posY; let scaleX; let
                scaleY;
        ...
        getFitPos(slide) {
            ...
            let canvasWidth; let canvasHeight; let minRatio; let width; let
                height;
        ...
        updateSlide(slide, duration) {
            const self = this;
            const $what = slide && slide.$content;
        ...
        centerSlide(slide, duration) {
            const self = this; let canvasWidth; let
                pos;
        ...
        updateCursor(nextWidth, nextHeight) {
            ...
            const $container = self.$refs.container.removeClass('fancybox-is-zoomable fancybox-can-zoomIn fancybox-can-drag fancybox-can-zoomOut');
        ...
        setImage(slide) {
            ...
            let found; let temp; let pxRatio; let
                windowWidth;
        ...
        revealContent(slide) {
            ...
            let effect; let effectClassName; let duration; let opacity; let end; let
                start = false;
            ...
            if (effect === 'zoom') {
                ...
                // Start animation
                $.fancybox.animate(slide.$content, end, duration, () => {
                    self.complete();
                });
            ...
            $.fancybox.animate($slide, 'fancybox-slide--current', duration, (e) => {
        ...
        getThumbPos(slide) {
            ...
            const isElementVisible = function ($el) {
                ...
                visibleInAllParents = parentRects.every((parentRect) => {
        ...
        preload(type) {
            const self = this;
            const next = self.slides[self.currPos + 1];
            const prev = self.slides[self.currPos - 1];
        ...
        close(e, d) {
            ...
            let effect; let
                duration;
            let $what; let opacity; let start; let
                end;
    ...
    function _run(e) {
        ...
        const opts	= e.data ? e.data.options : {};
        const value	= $target.attr('data-fancybox') || '';
        let index	= 0;
        let items = [];
...
(function ($) {
    ...
    $(document).on('objectNeedsType.fb', (e, instance, item) => {
        let url	 = item.src || '';
        let type = false;
        let media;
        let thumb;
...
(function (window, document, $) {
    ...
    const hasScrollbars = function (el) {
        ...
        const vertical = (overflowY === 'scroll' || overflowY === 'auto') && el.scrollHeight > el.clientHeight;
        const horizontal = (overflowX === 'scroll' || overflowX === 'auto') && el.scrollWidth > el.clientWidth;
    ...
    Guestures.prototype.onSwipe = function (e) {
        ...
        if (swiping === true) {
            ...
            if (Math.abs(self.distance) > 10) {
                ...
                $.each(self.instance.slides, (index, slide) => {
                    $.fancybox.stop(slide.$slide);
    ...
    Guestures.prototype.limitMovement = function () {
        ...
        let minTranslateX; let minTranslateY;
        let maxTranslateX; let maxTranslateY;
        let newOffsetX; let
            newOffsetY;
    ...
    Guestures.prototype.onZoom = function () {
        ...
        const endDistanceBetweenFingers = distance(self.newPoints[0], self.newPoints[1]);

        const pinchRatio = endDistanceBetweenFingers / self.startDistanceBetweenFingers;

        const newWidth = Math.floor(currentWidth * pinchRatio);
        const newHeight = Math.floor(currentHeight * pinchRatio);

        // This is the translation due to pinch-zooming
        const translateFromZoomingX = (currentWidth - newWidth) * self.percentageOfImageAtPinchPointX;
        const translateFromZoomingY = (currentHeight - newHeight) * self.percentageOfImageAtPinchPointY;

        // Point between the two touches

        const centerPointEndX = ((self.newPoints[0].x + self.newPoints[1].x) / 2) - $(window).scrollLeft();
        const centerPointEndY = ((self.newPoints[0].y + self.newPoints[1].y) / 2) - $(window).scrollTop();
        ...
        const newOffsetX = currentOffsetX + (translateFromZoomingX + translateFromTranslatingX);
        const newOffsetY = currentOffsetY + (translateFromZoomingY + translateFromTranslatingY);
    ...
    Guestures.prototype.endPanning = function () {
        const self = this;
        let newOffsetX; let newOffsetY; let
            newPos;
    ...
    Guestures.prototype.endZooming = function () {
        ...
        let newOffsetX; let newOffsetY; let newPos; let
            reset;
    ...
    Guestures.prototype.onTap = function (e) {
        ...
        const tapX = endPoints[0] ? endPoints[0].x - self.$stage.offset().left : 0;
        const tapY = endPoints[0] ? endPoints[0].y - self.$stage.offset().top : 0;
    ...
    $(document).on('onActivate.fb', (e, instance) => {
...
(function (document, $) {
    ...
    $.extend(SlideShow.prototype, {
        ...
        init() {
            ...
            self.$button = self.instance.$refs.toolbar.find('[data-fancybox-play]').on('click', () => {
                self.toggle();
            });
...
(function (document, $) {
    ...
    $(document).on({
        'onInit.fb': function (e, instance) {
            ...
            if (instance && instance.group[instance.currIndex].opts.fullScreen) {
                ...
                $container.on('click.fb-fullscreen', '[data-fancybox-fullscreen]', (e) => {
                    e.stopPropagation();
                    e.preventDefault();
    ...
    $(document).on(fn.fullscreenchange, () => {
    ...
    $.extend(FancyThumbs.prototype, {
        ...
        create() {
            ...
            self.$list.find('img').hide().one('load', function () {
                const $parent		= $(this).parent().removeClass('fancybox-thumbs-loading');
                const thumbWidth	= $parent.outerWidth();
                const thumbHeight	= $parent.outerHeight();
                let width;
                let height;
                let widthRatio;
                let heightRatio;
...
(function (document, $) {
    ...
    $(document).on('click', '[data-fancybox-share]', () => {
...
(function (document, window, $) {
    ...
    function parseUrl() {
        ...
        let index = rez.length > 1 && /^\+?\d+$/.test(rez[rez.length - 1]) ? parseInt(rez.pop(-1), 10) || 1 : 1;
...
(function (document, $) {
    ...
    $(document).on({
        'onInit.fb': function (e, instance, current) {
            instance.$refs.stage.on('mousewheel DOMMouseScroll wheel MozMousePixelScroll', (e) => {
                const current = instance.current;
